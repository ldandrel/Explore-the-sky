...
//    }));
    ...
    function VirtualSky(input) {
        ...
        this.projections = {
            ...
            'gnomic': {
                ...
                radec2xy: function (ra, dec, coords) {
                    ...
                    if (Math.abs(dec - this.dc_off) > this.maxangle) return {
                        x: -1,
                        y: -1,
                        el: -1
                    };
                    var ang = this.greatCircle(this.ra_off, this.dc_off, ra, dec);
                    if (ang > this.maxangle) return {
                        x: -1,
                        y: -1,
                        el: -1
                    };
                    ...
                    if (this.ground && coords[0] < -1e-6) return {
                        x: -1,
                        y: -1,
        ...
        r = br('0.5em');
        s = br('3px');
        $('<style type="text/css">' + v + '_help { ' + p + ':10px;' + c + ':white;' + r + '} ' + v + '_help ul { list-style:none;margin:0px;' + p + ':0px; } ' + v + '_infobox { ' + c + ':' + a + ';color:black;' + p + ':5px;' + r + bs + '} ' + v + '_infobox img {} ' + v + '_infocredit {color:white;float:left;font-size:0.8em;' + p + ':5px;position:absolute;} ' + v + 'form { position:absolute;z-index:20;display:block;overflow:hidden;' + c + ':#ddd;' + p + ':10px;' + bs + r + ' } ' + v + '_dismiss { float:right;' + p + ': 0 5px 0 5px;margin:0px;font-weight:bold;cursor:pointer;color:black;margin-right:-5px;margin-top:-5px; } ' + v + 'form input,' + v + 'form .divider { display:inline-block;font-size:1em;text-align:center;margin-right:2px; } ' + v + 'form .divider { margin-top: 5px; ' + p + ': 2px;} ' + v + '_help_key:active{ ' + k + ':#e9e9e9; } ' + v + '_help_key:hover{ border-color: #b0b0b0; } ' + v + '_help_key { cursor:pointer;display:inline-block;text-align:center;' + k + ':' + a + ';' + k + ':-moz-linear-gradient(top,' + a + ',' + b + ');' + k + ':-webkit-gradient(linear,center top,center bottom,from(' + a + '),to(' + b + '));' + s + '-webkit-' + k + '-clip:' + p + '-box;-moz-' + k + '-clip:' + p + ';' + k + '-clip:' + p + '-box;color:#303030;border:1px solid #e0e0e0;border-bottom-width:2px;white-space:nowrap;font-family:monospace' + ';' + p + ':1px 6px;font-size:1.1em;}</style>').appendTo("head");
    ...
    VirtualSky.prototype.init = function (d) {
        ...
        for (key in pairs)
            if (is(d[key], pairs[key])) this[key] = d[key];
    ...
    VirtualSky.prototype.loadLanguage = function (l, fn) {
        ...
        this.loadJSON(url, function (data) {
            this.langcode = l;
            this.langs[l] = data;
    ...
    VirtualSky.prototype.getPhrase = function (key, key2) {
        ...
        if (key === "constellations") {
            if (key2 && is(this.lang.constellations[key2], "string")) return this.htmlDecode(this.lang.constellations[key2]);
        } else if (key === "planets") {
    ...
    VirtualSky.prototype.setWH = function (w, h) {
        ...
        if (this.ie && $.browser.version == 8) $('#' + this.idinner).find('div').css({
            'width': w,
            'height': h
        });
        this.canvas.css({
    ...
    VirtualSky.prototype.load = function (t, file, fn) {
        return this.loadJSON(file, function (data) {
            ...
            this.draw();
            this.trigger("loaded" + (t.charAt(0).toUpperCase() + t.slice(1)), {
                data: data
            });
        }, fn);
    }
    ...
    VirtualSky.prototype.createSky = function () {
        ...
        this.times = this.astronomicalTimes();
        if (this.fntfam) this.container.css({
            'font-family': this.fntfam
        });
        if (this.fntsze) this.container.css({
            'font-size': this.fntsze
        });
        ...
        if (this.objects) {
            ...
            for (var o = 0; o < ob.length; o++) ob[o] = ((ob[o].search(/\.json$/) >= 0) ? {
                'url': ob[o],
                'src': 'file',
                'type': 'json'
            } : {
        ...
        if (this.c && this.c.getContext) {
            ...
            $("#" + this.idinner).on('click', {
                ...
                if (matched >= 0) $(e.data.sky.canvas).css({
                    cursor: 'pointer'
                });
            }).on('mousemove', {
                sky: this
        ...
        this.registerKey('8', function () {
            this.setClock('now').calendarUpdate();
        }, 'reset');
        this.registerKey('j', function () {
            if (!this.islive) this.spinIt("down");
        }, 'slow');
        this.registerKey('k', function () {
            this.spinIt(0)
        }, 'stop');
        this.registerKey('l', function () {
            if (!this.islive) this.spinIt("up");
        }, 'fast');
        this.registerKey('-', function () {
            this.setClock(-86400).calendarUpdate();
        }, 'subtractday');
        this.registerKey('=', function () {
            this.setClock(86400).calendarUpdate();
        }, 'addday');
        this.registerKey('[', function () {
            this.setClock(-86400 * 7).calendarUpdate();
        }, 'subtractweek');
        this.registerKey(']', function () {
            this.setClock(86400 * 7).calendarUpdate();
        }, 'addweek');
    ...
    VirtualSky.prototype.changeMagnitude = function (m) {
        ...
        this.magnitude += m;
        if (!this.starsdeep && this.magnitude > 4) this.load('stars', this.file.stars);
        else this.draw();
        return this;
    }
    VirtualSky.prototype.toggleHelp = function () {
        ...
        if ($('.' + v + '_dismiss').length > 0) $('.' + v + '_dismiss').trigger('click');
            ...
            for (var i = 0; i < this.keys.length; i++) {
                if (this.keys[i].txt) o += '<li>' + '<strong class="' + v + '_help_key ' + v + '_' + this.keys[i].txt + '">' + this.keys[i].str + '</strong> &rarr; <a href="#" class="' + v + '_' + this.keys[i].txt + '" style="text-decoration:none;">' + this.getPhrase(this.keys[i].txt) + '</a>' + '</li>';
            }
            $('<div class="' + v + '_help">' + '<div class="' + v + '_dismiss" title="' + this.getPhrase('close') + '">&times;</div>' + '<span>' + this.getPhrase('keyboard') + '</span>' + '<div class="' + v + '_helpinner"><ul></ul></div>' + '</div>').appendTo(this.container);
            var hlp = $('.' + v + '_help');
            ...
            $('.' + v + '_helpinner').css({
                'overflow': 'auto',
                'max-height': (this.tall - h) + 'px'
            });
            // Add the events for each keyboard option
            for (var i = 0; i < this.keys.length; i++) {
                if (this.keys[i].txt) $('.' + v + '_' + this.keys[i].txt).on('click', {
                    fn: this.keys[i].fn,
                    me: this
                }, function (e) {
                    e.preventDefault();
    ...
    VirtualSky.prototype.toggleInfoBox = function (i) {
        ...
        if ($('#' + this.id + '_' + this.infobox).length <= 0) this.container.append('<div id="' + this.id + '_' + this.infobox + '" class="' + this.infobox + '" style="display:none;"></div>');
    ...
    VirtualSky.prototype.selectProjection = function (proj) {
        if (this.projections[proj]) {
            ...
            if (this.container) {
                ...
                $('.' + this.id + '_projection').on('mouseover', {
                    me: this
                }, function (e) {
    ...
    VirtualSky.prototype.draw = function (proj) {
        ...
        this.drawGridlines("az").drawGridlines("eq").drawGridlines("gal").drawGalaxy().drawConstellationLines().drawConstellationLine(constellationId).drawConstellationBoundaries().drawStars().drawEcliptic().drawMeridian().drawPlanets().drawMeteorShowers().drawCardinalPoints();
        ...
        if (this.showhelp) {
            var helpstr = '?';
            if (d.find('.' + this.id + '_help').length == 0) d.append('<div class="' + this.id + '_help"><a href="#">' + helpstr + '</a></div>').find('.' + this.id + '_help').css({
                position: 'absolute',
                padding: 5,
                zIndex: 20,
                display: 'block',
        ...
        $('.' + this.id + '_position').css({
            ...
            if ($(hid + '_geo').length == 0) {
                ...
                s.container.append('<div id="' + id + '_geo" class="' + v + 'form">' + '<div id="' + id + '_geo_close" class="' + v + '_dismiss" title="' + s.getPhrase('close') + '">&times;</div>' + '<div style="text-align:center;margin:2px;">' + s.getPhrase('position') + '</div>' + '<div style="text-align:center;">' + '<input type="text" id="' + id + '_lat" value="" style="padding-right:10px!important;">' + '<div class="divider">' + s.getPhrase('N') + '</div>' + narrow + '<input type="text" id="' + id + '_long" value="" />' + '<div class="divider">' + s.getPhrase('E') + '</div>' + '</div>' + '</div>');
                $(hid + '_geo').css({
                    width: w,
                    'align': 'center'
                })
    ...
    VirtualSky.prototype.lightbox = function (lb) {
        ...
        function columize() {
            ...
            if (n > 1) {
                ...
                lb.find('li').css({
                    'width': (mx) + 'px',
                    'margin-left': Math.floor(bar / 2) + 'px'
                });
                lb.find('li:nth-child(' + n + 'n+1)').css({
                    'margin-left': '0px'
                });
            } else {
                lb.find('li').css({
            ...
            lb.find('ul').css({
                ...
                'width': Math.min(w - bar, (mx + bar / 2) * n + bar) + 'px'
            });
            lb.css({
                'z-index': 100,
                'position': 'absolute'
        ...
        lb.css({
            left: ((this.wide - lb.outerWidth()) / 2) + 'px',
            top: ((this.tall - lb.outerHeight()) / 2) + 'px'
        }).show();
        this.container.find('.virtualsky_dismiss').click({
            lb: lb,
            bg: bg
        }, function (e) {
            lb.remove();
    ...
    VirtualSky.prototype.drawConstellationLine = function (id) {
        if (!(this.constellationLine)) return this;
        if (id == undefined) return this;
        var colour = this.col.eq;
        var x = this.ctx;
        x.beginPath();
        x.strokeStyle = colour;
        x.fillStyle = colour;
        ...
        var pos, posa, posb, a, b, idx, l;
        var maxl = this.maxLine();
        for (l = 3; l < this.lines[id].length; l += 2) {
            a = -1;
            b = -1;
            idx1 = '' + this.lines[id][l] + '';
            idx2 = '' + this.lines[id][l + 1] + '';
            if (!this.hipparcos[idx1]) {
                for (s = 0; s < this.stars.length; s++) {
                    if (this.stars[s][0] == this.lines[id][l]) {
                        this.hipparcos[idx1] = s;
                        break;
                    }
                }
            ...
            if (!this.hipparcos[idx2]) {
                for (s = 0; s < this.stars.length; s++) {
                    if (this.stars[s][0] == this.lines[id][l + 1]) {
                        this.hipparcos[idx2] = s;
                        break;
                    }
                }
        ...
        pos = this.radec2xy(this.lines[id][1] * this.d2r, this.lines[id][2] * this.d2r);
        if (this.isVisible(pos.el)) {
            label = this.getPhrase('constellations', this.lines[id][0]);
            xoff = (x.measureText) ? -x.measureText(label).width / 2 : 0;
            x.fillText(label, pos.x + xoff, pos.y - fontsize / 2);
            x.fill();
        }
    ...
    VirtualSky.prototype.panTo = function (ra, dec, s) {
        ...
        this.panning = {
            s: {
                ra: this.ra_off * this.r2d,
                dec: this.dc_off * this.r2d
            },
            e: {
                ra: ra,
                dec: dec
    ...
    VirtualSky.prototype.advanceTime = function (by, wait) {
        if (by === undefined) {
            ...
            this.interval_calendar = window.setInterval(function (vs) {
                vs.calendarUpdate();
            }, 1000, this);
        }
        return this;
    ...
    VirtualSky.prototype.addPointer = function (input) {
        ...
        if (matched < 0) {
            ...
            if (typeof p.html !== "string") {
                ...
                p.html = p.html || '<div class="virtualsky_infocredit">' + '<a href="' + url + '" style="color: white;">' + credit + '</a>' + '</div>' + '<a href="' + url + '" style="display:block;' + style + '">' + '<img src="' + img + '" style="border:0px;' + style + '" title="' + label + '" />' + '</a>';
            }
            this.pointers[i] = p;
        }
    ...
    $.virtualsky = function (placeholder, input) {
        if (typeof input === "object") input.container = placeholder;
            ...
            if (typeof placeholder === "string") input = {
